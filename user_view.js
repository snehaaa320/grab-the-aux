...
function getValuesFromDB() {
    if (playlist_token !== null) {
        ...
        var queueRef = firebase.database().ref('Playlist_Table/' + playlist_token + '/Queue/').orderByChild('Upvotes');
...
function printToTable(song_name, song_artist, currentVote) {
    ...
    if (!is_guest) {
        ...
        played_button.innerHTML = '<button type="submit" name="song_played" onclick="removeSong(\'' + song_name + '\',\'' + song_artist + '\');">Yes</button>';
    }
}
...
function removeSong(song_name, song_artist) {
    ...
    while (table.rows.length > 1) {
        ...
        if (table.rows[1].cells[0].innerHTML != song_name) {
...
function hideHostFunctions() {
    ...
    song_table.rows[0].deleteCell(3);
}
